...
class MemoryBankEPI:
    """
    EPI mechanism that maintains pseudo-labels in a memory bank.
    Updates pseudo-labels directly using EMA without needing a teacher model.
    Equation: P_t = beta * P_{t-1} + (1 - beta) * p_t
    """
    def __init__(self, beta=0.6):
        self.beta = beta
        self.pseudo_labels = {}

    def update_and_get(self, image_names, current_preds):
        """
        image_names: list of unique filenames for the batch (e.g., from dataloader)
        current_preds: prediction output from the current model M_t
        """
        # Detach so gradients do not flow back into the pseudo-label memory bank
        current_preds = current_preds.detach().clone()
        batch_pseudo_labels = []
        
        for i, name in enumerate(image_names):
            p_t = current_preds[i]
            if name in self.pseudo_labels:
                P_t_minus_1 = self.pseudo_labels[name].to(p_t.device)
                # Apply the EPI formula
                P_t = self.beta * P_t_minus_1 + (1 - self.beta) * p_t
            else:
                # First time seeing this image, initialize with current prediction
                P_t = p_t
                
            # Store P_t on CPU to save GPU memory for large datasets
            self.pseudo_labels[name] = P_t.cpu()
            batch_pseudo_labels.append(P_t)
            
        return torch.stack(batch_pseudo_labels)


class ModelEPI:
    """
    EPI mechanism using an Exponential Moving Average (EMA) model (like Mean Teacher) 
    to generate P_{t-1}.
    """
    def __init__(self, model, alpha=0.99, beta=0.6):
        import copy
        self.alpha = alpha
        self.beta = beta
    ...
    @torch.no_grad()
    def update_ema_model(self, current_model):
        """ Update EMA model (M_{t-1}) """
        for ema_param, current_param in zip(self.ema_model.parameters(), current_model.parameters()):
            ema_param.data.mul_(self.alpha).add_(current_param.data, alpha=1 - self.alpha)

    @torch.no_grad()
    def generate_pseudo_labels(self, images, texts, current_preds):
        """
        Generate P_t from M_{t-1} and current_preds
        Note: You may need to apply sigmoid() or softmax() depending on whether 
        your model outputs raw logits or probabilities.
        """
        self.ema_model.eval()
        
        # P_{t-1} is the prediction of the generation model M_{t-1}
        P_t_minus_1 = self.ema_model(images, texts)
        
        # If the outputs are raw logits, apply activation
        P_t_minus_1 = torch.sigmoid(P_t_minus_1) 
        p_t = torch.sigmoid(current_preds.detach())
